...
// 导入API函数替代直接使用axios
import { getCancerChart, getUVByPostcode, getRecommendation } from '../api'
...
let uvChartInstance: Chart | null = null
...
const fetchPostcodeData = async () => {
  if (!postcodeError.value) {
    try {
      const response = await getUVByPostcode({
      ...
      // 存储UV数据以便后续使用
      uvData.value = data
      const hours = data.map((entry: UVDataEntry) => entry.hour)
      const uvIndex = data.map((entry: UVDataEntry) => entry.uvIndex)
